...
class ManageGitlabAccounts:
    ...
    def del_account(cls, account_id=None):
        ...
        if not all_gitlab_accounts:
            ...
            wf.add_item('Cannot delete account. No account was created', icon=ICON_WARNING)
            wf.send_feedback()
            return 1
        ...
        if deleted_acc:
            logger.info('Account "{}" was delete from dict.'.format(deleted_acc))
            wf.add_item('Account was delete from dict.', icon=ICON_INFO)
            wf.send_feedback()

        else:
            wf.add_item('Account for delete not exist.', icon=ICON_WARNING)
            wf.send_feedback()
            return 1
    ...
    def _set_pointer(cls, pointer):
        ...
        wf.save_password('gitlab_pointer', str(pointer))
